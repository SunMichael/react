...
-(UIView* )topBar{
    if(!topBar) {
        ...
        topBar.frame =CGRectMake(0, kScreenHeight-32, overlayWindow.frame.size.width, 30.0);
        topBar.alpha =1;
...
+(void)showTipMessageWithStatus:(NSString* )message{
    if (![StatusBar sharedView].showing) {
        [[StatusBar sharedView] showStatusWithString:message andTopImage:nil andTipIsBottom:NO];
        [StatusBar performSelector:@selector(dismiss) withObject:nil afterDelay:1.0f ];
    }
}
...
+(void)showTipMessageWithStatus:(NSString* )message andImage:(UIImage* )image andTipIsBottom:(BOOL)isBottom{
    if (![StatusBar sharedView].showing) {
        ...
        [StatusBar performSelector:@selector(dismiss) withObject:self afterDelay:1.0f ];
    }
...
+(void)showTipMessageWithStatus:(NSString* )message andImage:(UIImage* )image andCoin:(NSString*)coin andSecImage:(UIImage*)secImage andTipIsBottom:(BOOL)isBottom{
    if (![StatusBar sharedView].showing) {
        ...
        [StatusBar performSelector:@selector(dismiss) withObject:self afterDelay:1.0 ];
    }
...
-(void)showStatusWithString:(NSString* )string andTopImage:(UIImage *)image andTipIsBottom:(BOOL)isBottom{
    ...
    if (isBottom ==YES) {
        self.topBar.frame =CGRectMake((kScreenWidth - self.topImageView.frame.size.width -self.stringLabel.frame.size.width-25)/2, kScreenHeight- 37 -kFooterTabbarHeight, self.topImageView.frame.size.width+10+self.stringLabel.frame.size.width+15, 30);
    }else{
        self.topBar.frame =CGRectMake((kScreenWidth - self.topImageView.frame.size.width -self.stringLabel.frame.size.width-25)/2, kScreenHeight -kFooterTabbarHeight -37, self.topImageView.frame.size.width+10+self.stringLabel.frame.size.width+15, 30);
    }
...
-(void)showStatusWithString:(NSString* )string andTopImage:(UIImage *)image andCoin:(NSString*)coin andSecImage:(UIImage*)secImage andTipIsBottom:(BOOL)isBottom{
    ...
    self.topBar.frame =CGRectMake((kScreenWidth-self.topImageView.frame.size.width-self.stringLabel.frame.size.width-self.coinImageView.frame.size.width-35-self.coinLabel.frame.size.width)/2, kScreenHeight -37 -kFooterTabbarHeight, self.topImageView.frame.size.width+10+self.stringLabel.frame.size.width+25+self.coinImageView.frame.size.width+self.coinLabel.frame.size.width, 30);
