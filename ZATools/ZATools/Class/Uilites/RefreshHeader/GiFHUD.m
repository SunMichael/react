...
static UIImageView *animationView = nil;
+ (instancetype)instance {
    if (!instance) {
...
- (void)showAnimation{
    UIImage *icon = GetImage(@"loadgif0");
    if (animationView) {
        [animationView stopAnimating];
        [animationView removeFromSuperview];
        animationView = nil;
    }
    animationView = [[UIImageView alloc] initWithImage:icon];
    NSMutableArray *allImage = [NSMutableArray array];
    for (int i = 0; i <= 4; i++) {
        NSString *name = [NSString stringWithFormat:@"loadgif%d",i];
        [allImage addObject:GetImage(name)];
    }
    animationView.animationImages = allImage;
    animationView.animationDuration = 0.3f;
    animationView.animationRepeatCount = 0;
    float size = 70.0f;
    animationView.frame = CGRectMake(kScreenWidth/2 - size/2, kScreenHeight/2 -size/2, size, size);
